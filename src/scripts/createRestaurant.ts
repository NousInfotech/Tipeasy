...
const createWaiterData = (restaurantId: string, index: number) => ({
    ...
    ratings: 5,
    firebaseId: "", // Firebase ID will be generated by Firebase itself
...
const createRestaurantAndData = async () => {
    try {
        ...
        for (let i = 1; i <= 3; i++) {
            ...
            const waiter = await createWaiter(waiterData) as IWaiter;
...
// Execute the function to create the restaurant and data
createRestaurantAndData();
